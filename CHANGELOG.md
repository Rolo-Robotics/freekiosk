...
- 🖨️ **Web pages now print silently on a USB ESC/POS printer, with no print dialog.** A kiosk web app had no way to print unattended: Android's print framework always shows the system dialog, print services included, and nobody is standing at the tablet to tap it. `Settings > General > Printing` gains a **Print Destination Mode** dropdown — **Print Dialog**, the existing Android print dialog, or **Silent Print**, which drives an ESC/POS printer directly. Selecting Silent Print shows the attached printer's name, command set, `VID:PID` and paper state, with settings for print width, feed and cut, and a **test page** that proves the transport, the driver and the print width with no web app involved.

  For a web page the integration is `window.print()`, unchanged: the page's own print stylesheet is what lands on paper, so its fonts, its language and its QR codes come along, and nothing has to know FreeKiosk exists. Pages that want more get `window.FreeKiosk.printer` with `status()`, `printPage()` and `printImage()`. `status()` is the reason that API exists — a bare `window.print()` cannot tell a page the roll ran out, and a kiosk that reports a print that never came out is worse than one that says nothing. Rejections carry codes (`PAPER_OUT`, `NO_PRINTER`, `ORIGIN_NOT_ALLOWED`…), and a **Restrict printing by origin** switch limits printing to a list of origins; left off, any displayed page may print, as `window.print()` always has.
