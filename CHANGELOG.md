...
- 🖨️ **Receipts now print silently on a USB thermal printer, with no print dialog.** A kiosk web app that takes orders had no way to hand the customer a receipt: Android's print framework always shows the system dialog, print services included, and nobody is standing at an unattended tablet to tap it. `Settings > General > Printing` gains a **Print Destination** choice — the existing Android print dialog, or a **thermal receipt printer** driven directly. The thermal branch names the attached printer, its command set, its `VID:PID` and its paper state, with paper width (58 mm / 80 mm), feed, cut and a **test page** that exercises the whole path with no web app involved.
