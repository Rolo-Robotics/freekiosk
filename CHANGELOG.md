...
- 🖨️ **Receipts now print silently on a USB thermal printer, with no print dialog.** A kiosk web app that takes orders had no way to hand the customer a receipt: Android's print framework always shows the system dialog, print services included, and nobody is standing at an unattended tablet to tap it. `Settings > General > Printing` gains a **Print Destination** choice — the existing Android print dialog, or a **thermal receipt printer** driven directly. The thermal branch names the attached printer, its command set, its `VID:PID` and its paper state, with print width, feed, cut and a **test page** that exercises the whole path with no web app involved.

  For a web page the integration is `window.print()`, unchanged: the page's own print stylesheet is what lands on paper, so its fonts, its language and its QR codes come along, and nothing has to know FreeKiosk exists. Pages that want more get `window.FreeKiosk.printer` with `status()`, `printPage()` and `printImage()`. `status()` is the reason that API exists — a bare `window.print()` cannot tell a page the roll ran out, and a kiosk that reports a receipt it never printed is worse than one that says nothing. Rejections carry codes (`PAPER_OUT`, `NO_PRINTER`, `ORIGIN_NOT_ALLOWED`…), and an **Allowed origins** setting narrows who may call the API; left empty, any displayed page may print, as `window.print()` always has.

  Width is configured in **dots**, and that is the only paper setting stored: no dot pitch is assumed anywhere, and a printer whose specification is in millimetres is entered as printable width plus DPI and converted once. Pages are laid out so **one CSS pixel is one printer dot**, so a receipt is designed against the dot width in `px` rather than against physical units the browser and the printer would disagree about.
