...
- 🖨️ **Web pages now print silently on a USB thermal printer, with no print dialog.** A kiosk web app had no way to print unattended: Android's print framework always shows the system dialog, print services included, and nobody is standing at the tablet to tap it. `Settings > General > Printing` gains a **Print Destination Mode** dropdown — **Print Dialog**, the existing Android print dialog, or **Silent Print**, which drives a thermal printer directly. The thermal branch names the attached printer, its command set, its `VID:PID` and its paper state, with print width, feed, cut and a **test page** that exercises the whole path with no web app involved.
